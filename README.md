...
`app.py` のUIヘルパー関数（入力反映・表示分岐）のテスト。

- `tests/test_app_opcua_streamlit.py`
`app_opcua.py` の補助関数（モック入力・ノード取得・シナリオ制御）のテスト。
...
システムインストール済みパッケージも併用する場合（Raspberry Pi 環境向け）:

```bash
python3 -m venv --system-site-packages .venv
```

`--system-site-packages` を指定すると、仮想環境内から OS 側の `site-packages` を参照できます。  
OS 標準で入っているライブラリを利用したい場合に有効です。
...
- `app.py` と `app_opcua.py` は、`M001_SKIP_STREAMLIT_AUTORUN=1` を設定して import すると `render_app()` の自動実行を抑止できます（テストで利用）。
